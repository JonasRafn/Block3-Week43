...
public class WebScraperFacade {
    ...
    public List<Group> getGroups() throws InterruptedException, ExecutionException {
        ...
        List<Future<Group>> futureGroups = new ArrayList();
        ExecutorService executor = Executors.newFixedThreadPool(4);

        for (String url : urls) {
            Future<Group> future = executor.submit(new WebScraper(url));
            futureGroups.add(future);
        }

        for (Future<Group> future : futureGroups) {
            if (future != null) {
